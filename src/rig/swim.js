...
export class Swimmer {
  ...
  _recompute() {
    ...
    this.freq = Math.min(Math.max(f, 0.05), 6.5);
    this.omega = TAU * this.freq;
    this.k = TAU * sw.waves;
  }

  advance(dt) {
    // Re-read in case the GUI changed speed/strouhal since last frame.
    this._recompute();
    this.t += dt;
  }

  phase(s) {
    return this.k * s - this.omega * this.t;
  }

  /** Lateral (or dorsoventral) centreline offset in world units at coordinate s. */
  centreline(s) {
    const sw = this.swim;
    const amp = 0.5 * envelopeAt(sw.envelope, s) * this.L;
    let y = amp * Math.sin(this.phase(s));

    // A gentle constant curvature for steering, biased toward the tail.
    y += sw.turn * this.L * s * s * 0.5;
    ...
    if (sw.idle > 0) {
      y += sw.idle * 0.012 * this.L * Math.sin(this.omega * 0.14 * this.t + s * 1.3);
    }
    return y;
  }
  ...
  lateralVel(s) {
    const amp = 0.5 * envelopeAt(this.swim.envelope, s) * this.L;
  ...
  finDrive(s) {
    const tailAmp = 0.5 * envelopeAt(this.swim.envelope, 1.0) * this.L;
